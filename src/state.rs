...
impl Zeroize for RatchetState {
    fn zeroize(&mut self) {
        ...
        self.dh_sk.zeroize();
        self.dh_pk.zeroize();
        self.remote_dh_pk.zeroize();
        self.send_chain.zeroize();
        self.recv_chain.zeroize();
        self.send_header_chain.zeroize();
        self.recv_header_chain.zeroize();
        self.next_recv_header_chain.zeroize();
        self.prev_send_len.zeroize();

        for (_, key) in self.skipped_msg_keys.iter_mut() {
            key.zeroize();
        }
        self.skipped_msg_keys.clear();
        self.skipped_keys_fifo.clear();
        self.pending_kem_ciphertext.zeroize();
    }
}
