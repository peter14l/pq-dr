...
use crate::ratchet::{RatchetEngine, Message};
...
#[wasm_bindgen]
pub fn pqa_decrypt_wasm(state: &mut WasmRatchetState, header: &[u8], payload: &[u8], ad: &[u8]) -> Result<Vec<u8>, JsValue> {
    let message = Message {
        header_ciphertext: header.to_vec(),
        payload_ciphertext: payload.to_vec(),
    };

    RatchetEngine::decrypt(&mut state.inner, &message, ad)
