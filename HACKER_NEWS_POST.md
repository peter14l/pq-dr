...
1. It combines classical Curve25519 with Kyber (ML-KEM-768) for key encapsulation.
2. It uses Dilithium (ML-DSA) for authentication.
3. The cryptographic core is written in pure Rust (for performance and safety guarantees).
4. It exposes Flutter FFI bindings so mobile developers can integrate post-quantum E2EE with zero crypto boilerplate.

Under the hood:
- The server (built in Axum) manages PreKeys and handles cryptographic handshakes asynchronously.
...
- Performance implications of Dilithium signatures on mobile devices over high-latency connections.
...
It implements a hybrid scheme combining classical Curve25519 with Kyber (ML-KEM) and Dilithium (ML-DSA). If Kyber fails, the classical layer protects the message; if the classical layer is cracked by a quantum computer, Kyber holds the line.
