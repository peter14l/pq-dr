...
* `POST /prekey/upload` - Uploads a user's `PreKeyBundle`.
* `GET /prekey/fetch/:username` - Retrieves a target user's `PreKeyBundle`.
* `POST /message/send` - Enqueues an encrypted message for an offline user.
* `GET /message/fetch/:username` - Retrieves and flushes pending messages.
