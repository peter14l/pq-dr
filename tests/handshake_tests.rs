...
fn test_initial_message_serialization_roundtrip() {
    ...
    assert_eq!(initial_msg.alice_identity_pk, initial_msg2.alice_identity_pk);
    assert_eq!(initial_msg.ephemeral_pk, initial_msg2.ephemeral_pk);
    assert_eq!(initial_msg.kem_ciphertext_identity, initial_msg2.kem_ciphertext_identity);
}
...
fn test_multiple_handshakes_same_identity_keys() {
    ...
    for i in 0..3 {
        ...
        let msg = RatchetEngine::encrypt(&mut alice_state, format!("Session {}", i).as_bytes(), b"AD", &mut rng);
